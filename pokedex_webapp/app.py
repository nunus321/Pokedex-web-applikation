...
def pokemon_detail(pokedex_number):
    ...
    type_ids = [
        ...
        for t in conn.execute(
            "SELECT id FROM types t JOIN pokemon_types pt ON t.id = pt.type_id WHERE pt.pokemon_id = ?",
            (pokemon["id"],),
        ).fetchall()
    ]
...
def add_pokemon():
    if request.method == "POST":
        ...
        region_id = (
            int(request.form["region_id"]) if request.form["region_id"] else None
        )
        if not re.fullmatch(r"[A-Za-zÀ-ÿ♀♂'\-\s]+", name):
            flash(
                "Invalid Pokémon name. Only letters, spaces, hyphens, and gender symbols (♀♂) allowed."
            )
        ...
        base_attack = (
            int(request.form["base_attack"]) if request.form["base_attack"] else None
        )
        base_defense = (
            int(request.form["base_defense"]) if request.form["base_defense"] else None
        )
        base_special_attack = (
            int(request.form["base_special_attack"])
            if request.form["base_special_attack"]
            else None
        )
        base_special_defense = (
            int(request.form["base_special_defense"])
            if request.form["base_special_defense"]
            else None
        )
        base_speed = (
            int(request.form["base_speed"]) if request.form["base_speed"] else None
        )
        description = (
            request.form["description"] if request.form["description"] else None
        )
